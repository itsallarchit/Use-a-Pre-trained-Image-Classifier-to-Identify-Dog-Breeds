...
def check_creating_pet_image_labels(results_dic):
    ...
    if results_dic is None:
        ...
        n = 0
        for key in results_dic: # for loop to iterate through the dictionary
            # prints only first 10 labels
            if n < stop_point:
                print("{:2d} key: {:<30}  label: {:<26}".format(n+1, key, results_dic[key][0])) #left align
                n += 1 # Increments counter
            else:
                break
    ...
    if results_dic is None:
        ...
        for key in results_dic:
            ...
            if results_dic[key][2] == 1:
                ...
                n_match += 1
                print("{:>15}: Real: {:>13}   Classifier: {:>15}".format(key, 
